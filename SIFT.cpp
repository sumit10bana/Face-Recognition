...
void SIFT::print_all_descriptors_offline(int ctr)
{
...
//	scanf("%s",&name);
	printf("\n%d\n", m_keyDescs.size());
	for(int i = 1 ; i <= m_keyDescs.size(); i++) {	
		m_keyDescs[i-1].PrintFV_offline(ctr, i);
	}
}



string SIFT::Recognise()
{
	map<string, int> m;
		map<string, int> :: iterator it;
	//	printf("\n%d\n", m_keyDescs.size());
		for(int i = 0 ; i < m_keyDescs.size(); i++) {	
			string name2 = m_keyDescs[i].Recogn();
		//	cout <<name2<< endl;
			m[name2]++;
		}
		int tmp = 0;
...
string SIFT::Recognise1()
{
	map<string, int> m;
		map<string, int> :: iterator it;
	//	printf("\n%d\n", m_keyDescs.size());
		for(int i = 0 ; i < m_keyDescs.size(); i++) {	
			string name2 = m_keyDescs[i].Recogn1();
		//	cout <<name2<< endl;
			m[name2]++;
		}
		int tmp = 0;
...
void SIFT::print_all_descriptors()
{
	//for(int i =0 ; i < m_keyDescs.size(); i++) {
	printf("Enter name : " );
	char *name[20];
	scanf("%s",&name);
	FILE *file1;
	file1 = fopen("data1.txt", "a"); 
	printf("\n%d\n", m_keyDescs.size());
	for(int i = 0 ; i < m_keyDescs.size(); i++) {	
		m_keyDescs[i].PrintFV(name, file1);
	}
	fclose(file1);
}
...
void SIFT::BuildScaleSpace()
{
	...
	for(i=0;i<m_numOctaves;i++)
	{
		...
		for(j=1;j<m_numIntervals+3;j++)
		{
			...
			sprintf(filename, "C:\\SIFT\\Gaussian\\g_octave_%d_scale_%d.jpg", i, j);
		...
		if(i<m_numOctaves-1)
		{
			...
			// Store the image
			char* filename = new char[200];
			sprintf(filename, "C:\\SIFT\\Gaussian\\g_octave_%d_scale_0.jpg", i+1);
...
void SIFT::DetectExtrema()
{
	...
	for(i=0;i<m_numOctaves;i++)
	{
		...
		for(j=1;j<m_numIntervals+1;j++)
		{
			...
			// Save the image
			char* filename = new char[200];
			sprintf(filename, "C:\\SIFT\\Extrema\\extrema_oct_%d_scale_%d.jpg", i, j-1);
...
void SIFT::AssignOrientations()
{
	...
	for(i=0;i<m_numOctaves;i++)
	{
		...
		for(j=1;j<m_numIntervals+1;j++)
		{
			...
			char* filename = new char[200];
			sprintf(filename, "C:\\SIFT\\Mag\\mag_oct_%d_scl_%d.jpg", i, j-1);
			cvSaveImage(filename, magnitude[i][j-1]);

			sprintf(filename, "C:\\SIFT\\Ori\\ori_oct_%d_scl_%d.jpg", i, j-1);
	...
	for(i=0;i<m_numOctaves;i++)
	{
		...
		for(j=1;j<m_numIntervals+1;j++)
		{
			...
			// Save the regions!
			char* filename = new char[200];
			sprintf(filename, "C:\\SIFT\\Orientation Region\\ori_region_oct_%d_scl_%d.jpg", i, j-1);
...
void SIFT::ExtractKeypointDescriptors()
{
	...
	for(i=0;i<m_numOctaves;i++)
	{
		for(j=1;j<m_numIntervals+1;j++)
		{
			...
			char* filename = new char[200];
			sprintf(filename, "C:\\SIFT\\Interpolated Mag\\intmag_oct_%d_scl_%d.jpg", i, j-1);
			cvSaveImage(filename, imgInterpolatedMagnitude[i][j-1]);

			sprintf(filename, "C:\\SIFT\\Interpolated Ori\\intori_oct_%d_scl_%d.jpg", i, j-1);
